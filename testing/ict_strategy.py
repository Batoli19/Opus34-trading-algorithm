...
class ICTStrategy:
    ...
    def get_htf_bias(self, candles_h4: list) -> Direction:
        ...
        Multi-factor HTF bias using 3 independent methods voted together:
          ...
          1. EMA trend  — fast EMA vs slow EMA slope
          2. Price vs midline — close relative to 20-candle midpoint
          3. Candle structure — HH/HL vs LH/LL across swing segments

        Each method casts a vote (+1 bull, -1 bear, 0 neutral).
        Score >= +2  → BULLISH
        Score <= -2  → BEARISH
        Otherwise    → NEUTRAL

        Replaces the original single-condition check that required the current
        H4 candle to simultaneously break highs AND be above prior lows on the
        same bar — an extremely rare coincidence that caused the bias to return
        NEUTRAL almost 100% of the time, blocking every downstream signal.
        """
        ...
        # Fast EMA (8) vs slow EMA (21) — if fast > slow, trend is up
        ...
        if fast_ema > slow_ema * 1.0001:   # fast meaningfully above slow
            score += 1
        elif fast_ema < slow_ema * 0.9999: # fast meaningfully below slow
            score -= 1

        # ── Method 2: Price vs range midpoint ─────────────────────────────────
        # Is the current close in the upper or lower half of the 20-candle range?
        range_high = max(highs[-20:])
        range_low  = min(lows[-20:])
        midpoint   = (range_high + range_low) / 2
        last_close = closes[-1]

        band = (range_high - range_low) * 0.1   # 10% band around mid = neutral
        ...
        # ── Method 3: Swing structure (HH/HL vs LH/LL) ───────────────────────
        # Divide last 20 candles into 4 segments of 5, compare segment highs/lows.
        # This is trend-of-swings rather than single-candle comparison.
        seg_size = 5
